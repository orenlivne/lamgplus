...
graphs. LAMG+ and AC are the only solvers that converge across all thirteen test classes, while BoomerAMG, PETSc, pyAMG, and CMG each slow by an order of magnitude or fail to converge off their home turf. (2)
\emph{Linear scaling}: LAMG+ is empirically $\Om$ over the full $1{,}711$-graph SuiteSparse set
($100\%$ converged, median $4$ cycles, log--log slope $1.01$), verified to $2.4\!\times\!10^8$. (3)
...
implementation, not the algorithm. The two refinements, a strength-of-connection aggregation veto and a selective caliber-2 interpolation, resolve LAMG's grid-aligned-anisotropy convergence failure (asymptotic convergence factor $\approx0.99\!\to\!0.11$). Both trigger locally and do not substantially increase the complexity. Code and benchmark scripts: \url{https://github.com/orenlivne/lamgplus}.
\end{abstract}
...
substantially more robust by Gao, Kyng, and Spielman \cite{gks2023}, and paralleled by randomized-Cholesky direct
...
\cite{learned-amg}. LAMG \cite{lamg}, by contrast, is parameter-free and training-free. Combinatorial
Multigrid (CMG) \cite{cmg} also targets general topologies; CMG and LAMG had comparable average
speed but LAMG was markedly more robust. General-purpose aggregation packages
(PyAMG \cite{pyamg}, hypre/BoomerAMG \cite{boomeramg}) are tuned for PDE discretizations rather
...
(\S\ref{sec:families})---a difference specific to that implementation, not the algorithm. We thus consider
approxChol as our sole robust near-linear baseline, with CHOLMOD for the
direct-method crossover. We also compare with the other AMG solvers for the specific graph classes they converge on.
...
(\S\ref{sec:compete}). LAMG (2012) predates and was never compared with Spielman's approxi\-mate-Cholesky
...
$1{,}711$ SuiteSparse graphs---$100\%$ converged, median $4$ cycles, log--log slope $1.01$ up to $2.3\!\times\!10^8$ edges (twice larger than LAMG's original demonstration) with no tuning.
\item \emph{Robustness, closing LAMG's outlier tail} (\S\S\ref{sec:choices},\,\ref{sec:families}).
Where the original MATLAB LAMG was reported by \cite{gks2023} as non-convergent across the benchmark's test families, our
re-derivation converges to their $10^{-8}$ tolerance on all of them---proving that the gap is specific to
that implementation, not the algorithm.
Additionally, two lean, \emph{local} refinements---a strength-of-connection veto and a selective caliber-2
interpolation---turn LAMG's stalled
...
$L^c e^c=P^{\top}(b-L\tilde\phi)$ with $L^c=P^{\top}LP$ and $\tilde\phi$ the approximation to $\phi$ after relaxation, and corrects
...
scores $u$ forced onto seed $s$, $q_{u\leftarrow s}$, and is not symmetrized in $(u,s)$. These
...
\caption{The two places LAMG+ departs from LAMG, both targeting grid-aligned anisotropy. Reproduced setup choices (no size cap, orphan singletons, directional guard,
...
$2{:}1$ aggregate recovers only half of the smooth-in-strong mode's amplitude, inflating the energy ratio to
$q\approx2$ and pinning the two-level factor at $\rho\approx0.5$ (App.~\ref{sec:aniso}). Where a
fine node's \emph{strong} edges (i.e., those not vetoed by~\eqref{eq:soc}) reach exactly two coarse aggregates---a locally one-dimensional
neighborhood---LAMG+ adds a second, caliber-2 parent with a single fitted weight $w$
...
SuiteSparse graphs ($50\le n\le10^6$, $52 \le m \le 9.8\!\times\!10^5$). Every one converges to $\varepsilon=10^{-8}$, in a median of $4$ cycles. The log--log slopes of per-edge time are $+0.005$ (setup)
...
solver is paced by iteration count; the winner is whichever builds the \emph{leaner hierarchy},
because the fill a solver creates sets both its setup cost and the cost of every cycle. Aggregation
and sampled elimination have \emph{opposite} fill profiles. On low-diameter social, web, and citation
...
$\Oone$ cost on a cleanly separating mesh---and so does the order: \textbf{LAMG+ is the fastest of the three} (geometric-mean
...
correlation of $\log\bar d$ with a LAMG+ win is $r=+0.68$. The mechanism is two cost axes keyed to properties that anti-correlate across this
corpus: approximate-Cholesky's fill grows with elimination-clique size, hence with degree, while
LAMG+'s operator complexity grows with poor separability (low-diameter, small-world structure)---and
here the well-separated graphs are the high-degree ones, so $\bar d$ proxies both. High $\bar d$
favors LAMG+ and low $\bar d$ favors sampled elimination; the residual $9\%$ are exactly the graphs
where the two axes disagree---high-degree but small-world, or low-degree but cleanly separable.
...
Table~\ref{tab:classcomp} widens the comparison to all seven solvers across thirteen graph classes,
...
specific to that implementation, not the algorithm; our Julia implementation matches the original's design
(\S\ref{sec:alg}) line by line.
...
only second solver, alongside AC, that is robust across all twelve families. On these well-structured families approxChol is faster---bounded degrees and separators
...
$10^5$ cores \cite{baker-scaling,amgx}---but the one part that seems inherently sequential is
the Gauss--Seidel smoother, since each update reads its already-updated neighbors. One solution is to replace it with polynomial smoothers \cite{adams-poly}. Instead, we propose a parallel GS version.

\paragraph{Parallel GS (a smoothing-factor analysis)} The standard parallel
...
The algebraic equivalent of the smoothing factor \cite[\S\S3.1,\,3.3]{guide} is $\mu=\|P_{\mathrm{HF}}\,S\,P_{\mathrm{HF}}\|_2$, where $S=I-M^{-1}A$ and
$P_{\mathrm{HF}}$ is the orthogonal projector onto the eigenvectors of $A$ above its median eigenvalue
(on a regular grid, the upper Fourier quadrant). The reference values for a 2D grid graph are $\mu{=}1$ (undamped Jacobi,
...
The proposition implies that, at least on a grid, freezing the cross-block seams perturbs the smoother to first order in the cut weight $1/W$, so the parallel smoother retains most of the serial smoother's power. Three independent measurements (the operator norm, local Fourier analysis (grids only), and direct
high-frequency sweeps) illustrate this: at the operative
...
reach the pure-Jacobi non-smoother $\mu{=}1$. Note that the cut-edge fraction does not predict the penalty: the
...
partitioned to minimize inter-processor communication---itself a multilevel graph-partitioning problem
...
whose cost is set by the highest-degree boundary nodes. LAMG+'s coarsening is, however, more local: the
per-node affinity and caliber-2 tests read only a node's immediate neighborhood, so they distribute
naturally once a partition is fixed. Whether that locality preserves LAMG+'s lean operator complexity
under a communication-minimizing partition is an open question. Nothing in the algorithm is inherently sequential---a no-fill aggregation hierarchy
...
finite-element/structural matrices. Beyond the linear solver, the multilevel hierarchy could be used for related tasks such as eigenproblems \cite[\S6.4]{lamg}, nonlinear systems via the Full Approximation Scheme \cite[\S8.1]{guide}, and has more parallelism potential than factorization methods.
...
near-geometric rate. ACF $=$ asymptotic convergence factor; the number in parentheses is the
cycle count to reach $10^{-10}$.}\label{tab:aniso}
\begin{tabular}{lccc}
\toprule
weak weight $\epsilon$ & caliber-1 ($\gamma{=}1.5$+recomb.) & \textbf{caliber-2} & flat V-cycle\\
\midrule
$1$ (isotropic)   & 0.010 (5)  & 0.008 (4)          & 0.74 (40)\\
$10^{-1}$         & 0.274 (11) & \textbf{0.007} (4) & 0.76 (44)\\
$10^{-2}$         & 0.335 (14) & \textbf{0.051} (5) & 0.77 (53)\\
$10^{-4}$         & 0.391 (15) & \textbf{0.100} (6) & 0.77 (56)\\
\bottomrule
\end{tabular}
\end{table}
...
\emph{same} $K$ test vectors used for affinity (no extra setup): minimize
...
\paragraph{The role of caliber-2} The guard $Q$ of \eqref{eq:guard} already makes the solver robust in every
