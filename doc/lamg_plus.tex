...
\paragraph{Does a richer model help?} The residual $9\%$ invites a learned selector. We tested one: a
gradient-boosted decision tree (a histogram model of the LightGBM class) trained on \emph{thirteen}
...
attains $89\pm4\%$---\emph{not} better than the one-line $\bar d$ threshold ($91\%$)---and a tree
given $\bar d$ alone scores the same, with permutation importance concentrated almost entirely on
$\bar d$ (every shape feature contributes $\le0.01$). The degree \emph{distribution} thus carries no
...
\paragraph{Toward a parallel LAMG+} The results here are single-threaded; parallelization is the main
direction left open. Both the setup and the cycle reduce to scalable sparse primitives---sparse
...
caliber-2 can be \emph{derived}, not only measured. On the constant-coefficient model---strong weight
$1$ along $x$, weak weight $\epsilon$ along $y$ on $\mathbb Z^2$---the Laplacian acts on the grid mode
$\varphi_\theta(\mathbf m)=e^{\mathrm{i}\,\theta\cdot\mathbf m}$ with symbol
\begin{equation}\label{eq:Lsymbol}
\hat L(\theta)=4\sin^2\tfrac{\theta_x}{2}+4\epsilon\sin^2\tfrac{\theta_y}{2},
\qquad \theta=(\theta_x,\theta_y)\in(-\pi,\pi]^2 .
\end{equation}
As $\epsilon\ll1$, $\hat L(\theta)$ is small exactly when $\theta_x\approx0$ for \emph{any} $\theta_y$:
the algebraically smooth error is smooth along the strong axis but unconstrained across the weak one.
Affinity therefore semicoarsens $2{:}1$ in $x$ and keeps every $y$-line---the correct coarsening, since
full $y$-resolution represents the $\theta_y$-oscillatory slow modes (Fig.~\ref{fig:semicoarsen}).

\emph{Harmonics.} The $2{:}1$ $x$-coarsening folds each low mode $\theta$
($\theta_x\in(-\tfrac\pi2,\tfrac\pi2]$) with its $x$-harmonic
$\theta^{\ast}=(\theta_x{+}\pi,\,\theta_y)$; the pair spans a subspace invariant under the two-grid
operator, on which the operator is the $2\times2$ matrix
\begin{equation}\label{eq:tgsymbol}
\hat M(\theta)=\hat S(\theta)^{\nu}\Big[\,I-\hat P\,(\hat P^{\ast}\hat L\hat P)^{-1}\hat P^{\ast}\hat L\,\Big],
\qquad \hat L=\mathrm{diag}\big(\hat L(\theta),\hat L(\theta^{\ast})\big)=:\mathrm{diag}(\ell_1,\ell_2),
\end{equation}
with $\hat S$ the smoother symbol, $\nu=\nu_{\mathrm{pre}}{+}\nu_{\mathrm{post}}$ the number of smoothing sweeps, $\hat P\in\mathbb C^{2\times1}$
the interpolation symbol, Galerkin coarse symbol $\hat L_c=\hat P^{\ast}\hat L\hat P$, and variational
restriction $\hat R=\hat P^{\ast}$. The bracketed coarse-grid correction is the $\hat L$-orthogonal
projector \emph{off} $\mathrm{range}\,\hat P$, hence idempotent with spectrum $\{0,1\}$: correction
alone does not converge---the smoother must remove whatever the coarse space omits.

\emph{Interpolation symbols.} Write $s=\sin^2\tfrac{\theta_x}{2}$, $c=\cos^2\tfrac{\theta_x}{2}$
($s{+}c=1$). Interpolating the fine point midway between two coarse parents has the symbol (up to a
...
\end{equation}
The decisive difference is the \emph{harmonic} (second) component as $\theta_x\to0$: it is $O(\theta_x)$
for caliber-1 but $O(\theta_x^2)$ for caliber-2. Caliber-2 injects asymptotically no energy into the
unresolved harmonic of a smooth mode---the order-$2$ interpolation property \cite[\S4.3]{guide}.

\emph{The factor.} The correction in \eqref{eq:tgsymbol} is a rank-one projector, so $\det\hat M=0$
and the spectral radius equals the trace:
\begin{equation}\label{eq:rhotrace}
\rho\big(\hat M(\theta)\big)=\big|\,\hat S(\theta)^{\nu}(1-\beta)+\hat S(\theta^{\ast})^{\nu}\,\beta\,\big|,
\qquad
...
\qquad
\beta_{\mathrm{c2}}=\cos^2\tfrac{\theta_x}{2}\to1\ \ (q\to1)\ \text{ as }\theta_x\to0 :
\end{equation}
...
\emph{The smoother.} We use the cycle's own relaxation---forward Gauss--Seidel ($\omega{=}1$)---with
symbol $\hat S(\theta)=1-\hat L(\theta)/\hat M(\theta)$, where
$\hat M(\theta)=(2{+}2\epsilon)-e^{-\mathrm{i}\theta_x}-\epsilon\,e^{-\mathrm{i}\theta_y}$ is its
lower-triangular (already-updated-neighbor) part; the solver applies it
$\nu=\nu_{\mathrm{pre}}{+}\nu_{\mathrm{post}}=3$ times per visit (its $(1,2)$ cycle). The symbol is
complex, so we evaluate \eqref{eq:rhotrace} numerically---but the energy ratio $q$ fixes the outcome.

\emph{The caliber-1 ceiling.} The supremum of \eqref{eq:rhotrace} is attained as $\theta_x\to0$, on modes
\emph{smooth in $x$}, where any local smoother is powerless ($|\hat S(\theta)|\to1$) while the
$x$-harmonic is fully smoothed ($\hat S(\theta^{\ast})\to0$). There $\rho\to1-\beta$, so
...
\end{equation}
an irreducible ceiling sitting on an unsmoothable mode: neither more sweeps nor a stronger smoother
lowers it. Forward Gauss--Seidel realizes $\sup_\theta\rho_{\mathrm{c1}}\approx0.48$ for
$\epsilon\le10^{-1}$, matching the directly measured caliber-1 two-grid factor on a periodic grid,
$\approx0.47$ (Table~\ref{tab:aniso}).
...
gives $\sup_\theta\rho_{\mathrm{c2}}\approx0.08$--$0.09$ for $\epsilon\le10^{-1}$, an order of magnitude
below the caliber-1 ceiling, and the directly measured periodic-grid two-grid factor agrees
(Table~\ref{tab:aniso}, ``2-level'': $0.08$--$0.12$; the small excess is the least-squares weight's
deviation from the ideal $\tfrac12$). Crucially this two-grid factor ($\approx0.1$) is \emph{already}
the order of the full multilevel solver's caliber-2 rate ($0.007$--$0.1$): with the cycle's own smoother
the two-grid analysis predicts the solver directly, so $\gamma$-cycling and recombination adjust the
rate only modestly---there is no large gap for them to close. (On a finite grid the boundary columns
reach a single coarse aggregate and stay caliber-1, raising the two-grid factor to $\approx0.2$;
recombination and the multilevel recursion remove that residue.) Caliber-1, by contrast, never clears
its $q{=}2$ ceiling even as a full multilevel cycle.

\emph{Scope.} The analysis is constant-coefficient: it proves caliber-2's textbook efficiency on a
\emph{correctly} semicoarsened grid, and so justifies raising the caliber by the interpolation-order
rule \cite[\S4.3]{guide} rather than by experiment. It does not see the finite-sample affinity
inversion of \S\ref{sec:soc}, which corrupts the coarsening itself on an unlucky test-vector sample;
the strength-of-connection veto is what secures the clean semicoarsening this analysis assumes, so the
two refinements are complementary by construction.

\begin{table}[h]\centering\footnotesize\setlength{\tabcolsep}{4pt}
\caption{Controlled anisotropy on a $64\times64$ grid: asymptotic convergence factor by method, all
using the cycle's relaxation---forward Gauss--Seidel, $\omega{=}1$,
$(\nu_{\mathrm{pre}},\nu_{\mathrm{post}}){=}(1,2)$. \emph{V-cycle} $=$ flat $\gamma{=}1$ cycle and
\emph{ML} $=$ the full solver ($\gamma{=}1.5$ $+$ recombination), both the full multilevel solver on the
finite grid. The two-grid columns---\emph{LFA} (the analytic factor of \S\ref{sec:lfa}), \emph{2-level}
(one coarsening, exact coarse solve), and \emph{$+$recomb} (the same with recombination)---are measured
on a \emph{periodic} grid, the local-Fourier setting, so LFA and 2-level are directly comparable: they
agree (caliber-1 $0.48$ vs $0.47$; caliber-2 $\approx0.09$ vs $0.08$--$0.12$, the small excess being the
least-squares weight vs the ideal $\tfrac12$). That two-grid factor ($\approx0.1$) is \emph{already} the
order of the full solver, so recombination and the multilevel recursion change it little. (On the finite
grid the boundary columns stay caliber-1, raising the two-grid factor to $\approx0.2$; the full solver
removes that residue.) Caliber-1 never clears its $q{=}2$ ceiling; caliber-2 is inert at $\epsilon{=}1$
(isotropic, $0\%$ upgrade); ML columns reach $10^{-10}$ in $4$--$15$ cycles.}\label{tab:aniso}
