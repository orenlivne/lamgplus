...
aggregate-quality control \cite{napov-notay} (the closest algorithmic relative; discussed in \S\ref{sec:draqc}), matching-based multilevel preconditioners
...
condition-number bound rather than an energy guard, and is reported $2$--$5\times$ faster than the
original LAMG on a large real-world test set \cite{napov-notay}.

\paragraph{Availability of competing implementations}
The two most closely related aggregation solvers are not openly available. AGMG \cite{notay}---the
general-purpose quality-control aggregation code---is distributed under a restrictive academic
licence, which we requested and were unable to obtain as a non-academic user; its degree-aware graph
variant DRA-QC is not part of any public distribution. We reimplemented DRA-QC faithfully from
\cite{napov-notay} during this study, but we deliberately omit head-to-head timings against
it: a third-party reimplementation cannot fairly represent the authors' tuned code, and a comparison
against our own version would be neither reliable nor fair to them. We therefore benchmark
