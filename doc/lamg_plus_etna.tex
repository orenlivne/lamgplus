\documentclass[leqno,letterpaper]{etna}
...
\title{A Reproducible Comparison of Graph-Laplacian Solvers,
  with Robustness Refinements to Lean Algebraic Multigrid\thanks{%
  Received \today. Accepted \ldots. Published online on \ldots.
  Recommended by \ldots.}}

\author{Oren E.~Livne\footnotemark[2]}

\shorttitle{LAMG+: A ROBUST LEAN ALGEBRAIC MULTIGRID}
\shortauthor{O.~E.~LIVNE}

\begin{document}
\maketitle

\renewcommand{\thefootnote}{\fnsymbol{footnote}}
\footnotetext[2]{Pine Birch Analytics, 35 Kelinger Rd, Churchville,
  PA~18966-1033 (\texttt{oren.livne@gmail.com},
  \texttt{pinebirchanalytics.com};
  ORCID: \href{https://orcid.org/0000-0001-6700-483X}{0000-0001-6700-483X}).}
...
learning, finite-element analysis, and interior-point network-flow solvers. We present a
\emph{reproducible, same-machine study} of graph-Laplacian solvers centered on \textbf{LAMG+}---an open,
parameter-free Julia re-derivation of Lean Algebraic Multigrid (LAMG) with two targeted robustness
refinements---benchmarked under identical conditions against both approximate-Cholesky variants, the
degree-aware aggregation of Napov and Notay, and five further solvers. Three findings emerge.
First, \emph{same-machine benchmarking} against both approximate-Cholesky (AC) variants---the modern
near-linear champion---and five further solvers (hypre/BoomerAMG, PETSc GAMG, pyAMG, CMG, CHOLMOD)
shows LAMG+ and AC are \emph{complementary peers}: AC is faster on social and
...
graphs. LAMG+ and AC are the only solvers that converge across all 13 test classes, while BoomerAMG, PETSc, pyAMG, and CMG each slow by an order of magnitude or fail to converge off their home turf.
Napov and Notay's degree-aware aggregation (DRA-QC) is likewise robust and a complementary competitor measured in work per edge---leaner than LAMG+ on the typical graph, heavier on grid-aligned anisotropy---assessed from a faithful reimplementation, no public code being available; among all three robust solvers, LAMG+ is the only one convergent on every graph.
Second, \emph{linear scaling}: LAMG+ is empirically $\Om$ over the full $1{,}711$-graph SuiteSparse set
($100\%$ converged, median $4$ cycles, log--log slope $1.01$), verified to $2.4\!\times\!10^8$ nonzeros.
Third, \emph{robustness}: the original MATLAB LAMG was reported non-convergent across the
AC benchmark's test families, yet LAMG~2.2.1 (under GNU Octave; algorithm and MEX kernels unchanged), run on those
families with the benchmark's own solver call, converges on every one---as does LAMG+,
to their $10^{-8}$ tolerance. The two refinements, a strength-of-connection aggregation veto and a selective caliber-2 interpolation, resolve LAMG's grid-aligned-anisotropy convergence failure (asymptotic convergence factor $\approx0.99\!\to\!0.11$). Both trigger locally and do not substantially increase the complexity. Code and benchmark scripts: \url{https://github.com/orenlivne/lamgplus}.
\end{abstract}
...
aggregate-quality control (DRA-QC) \cite{napov-notay}, which controls aggregate quality by a
provable two-grid bound rather than an energy guard and is the solver most closely related to
LAMG (our closest competitor, benchmarked in \S\ref{sec:draqc}); matching-based multilevel preconditioners
...
non-convergent there, whereas both LAMG~2.2.1 (under GNU Octave) and our independent re-derivation
converge on those families (\S\ref{sec:families}). We thus consider
...
original MATLAB reference) that we use to establish four things.
...
test families, we run LAMG~2.2.1 (under GNU Octave; algorithm and MEX kernels unchanged) on those
families with the benchmark's exact solver call and find it converges on every one, as does our
re-derivation to the $10^{-8}$ tolerance (Table~\ref{tab:lamgorig}).
...
(18-core, 48~GB RAM). The Julia-native solvers (approxChol, AC, CHOLMOD via Julia wrappers) run
in the same process under identical JIT compilation. The non-Julia solvers (pyAMG, BoomerAMG,
PETSc GAMG, CMG) are invoked as subprocesses through their native libraries, but each is timed
\emph{internally}---after a warm-up call, excluding interpreter and library startup---so every
solver's reported number is the same quantity, setup$+$solve, with wall-clock as the comparison
metric used by \cite{gks2023}.
...
variants: the original, faster \texttt{approxchol\_lap} (``approxChol'') \cite{laplacians}
and the robust \texttt{approxchol\_lap2} (``AC'') of \cite{gks2023}, and note sparse
Cholesky (CHOLMOD) \cite{davis-cholmod} for context.\end{sloppypar} For these three Julia-native solvers, any timing difference is algorithmic (coarsening vs.\ fill), not a compiler artifact; we
...
\subsection{The closest competitor: degree-aware aggregation}\label{sec:draqc}
The graph-Laplacian solver most closely related to LAMG is the degree-aware rooted aggregation
with quality control (DRA-QC) of Napov and Notay \cite{napov-notay}: like LAMG it eliminates
low-degree vertices and coarsens by aggregation, but it controls aggregate quality through a
provable two-grid condition-number bound rather than an energy guard, and is reported $2$--$5\times$
faster than the original LAMG on a large real-world test set. No public implementation of this
graph-Laplacian-specialized algorithm exists and we were unable to obtain the authors' code; we
therefore reimplemented DRA-QC faithfully from the
published description---validated against its published operator complexities and
$15$--$30$-iteration range---and emphasize that \emph{all DRA-QC figures below are from this
reimplementation}, not the original code, and should be read accordingly.
(The production package AGMG~\cite{agmg} implements the \emph{general-purpose} quality-control
aggregation with guaranteed convergence rate~\cite{napov-notay-2012}---the ``QC'' lineage from which
DRA-QC descends---but its default coarsening is the general elliptic-PDE scheme, not the
graph-Laplacian-specialized degree-aware rooted variant evaluated here; it is thus a relative of
DRA-QC, not a public implementation of it.) All comparisons in this
section are \emph{work per edge} (total floating-point operations divided by $m$), not wall-clock
time, so they are independent of implementation quality. We ran all three robust near-linear
solvers under identical build-once conditions over the full SuiteSparse corpus of \S\ref{sec:scaling}.

All three are robust: each converges on over $99.7\%$ of the corpus
(Table~\ref{tab:draqc}). LAMG+ is the only one that converges on \emph{every} graph, with the
tightest tail (at most $27$ cycles; approxChol reaches $77$). DRA-QC's tail is wider on
anisotropic/Darcy/thermal matrices---an expected consequence of aggregation-only coarsening without
a caliber-2 fix---though the one extreme outlier in Table~\ref{tab:draqc} should be interpreted
cautiously given that it comes from our reimplementation. Measured in work per edge,
the two are \emph{complementary}: DRA-QC's leaner cycle does \emph{less} total work than LAMG+ on the
typical graph (median ratio $0.68$), but substantially more on the $11\%$ of graphs with grid-aligned
anisotropy or high contrast (up to $55\times$), while LAMG+'s worst-case work stays bounded.
DRA-QC is a strong competitor with a complementary cost profile and a proven two-grid bound.
...
(LAMG+), PCG iterations (approxChol), or K-cycle iterations (DRA-QC---our faithful
reimplementation; the tail reflects an aggregation-only method's known anisotropy limitation
and should be read with the usual caveat that reimplementation outliers may not reflect the
original code).}\label{tab:draqc}
...
DRA-QC \cite{napov-notay} & 99.8\%           & 13 & 89 & 277          & 4\\
\bottomrule
\end{tabular}
\end{table}
...
attains $89\pm4\%$ (cross-validated)---no better than the one-line $\bar d$ threshold; note that
the $91\%$ for the threshold is in-sample (both chosen and evaluated on the same $201$ graphs),
so the two numbers are not directly comparable, but the permutation importance concentrates almost
entirely on $\bar d$, every other feature contributing $\le0.01$. The degree distribution thus carries no
...
\cite{gks2023}. We ran LAMG~2.2.1 under GNU Octave (compatibility shims only; the multigrid
algorithm, setup, and MEX kernels are unchanged) with \cite{gks2023}'s exact solver call
(\texttt{setup(`laplacian',$\cdot$)}, \texttt{errorReductionTol}$=10^{-8}$, default cycle cap) on the
same families and find it converges on every one (Table~\ref{tab:lamgorig}), as does
LAMG+, in as few or fewer cycles and to a strict $10^{-8}$ residual.

% Table generated by scripts/gks_family_compare_table.jl (see doc/gks_compare_table.tex); regenerate, don't hand-edit.
\begin{table}[t]\centering
\caption{LAMG~2.2.1 (run under GNU Octave; algorithm and MEX kernels unchanged) vs.\ LAMG+ on the
families of the Gao-Kyng-Spielman (GKS) benchmark \cite{gks2023}, \emph{same graph and right-hand side}.
Both converge on every instance. LAMG+ reaches a strict
...
\textbf{37}, 2010.
\bibitem{agmg} Y.~Notay, \emph{AGMG software and documentation},
\texttt{http://agmg.eu}.
\bibitem{napov-notay-2012} A.~Napov and Y.~Notay, \emph{An algebraic multigrid method with
guaranteed convergence rate}, SIAM J.\ Sci.\ Comput.\ \textbf{34}(2), 2012.
