...
degree-aware aggregation of Napov and Notay, and four further state-of-the-art solvers. We establish three facts. (1)
...
($100\%$ converged, median $4$ cycles, log--log slope $1.01$), verified to $2.4\!\times\!10^8$ nonzeros. (3)
...
Cholesky (CHOLMOD) \cite{davis-cholmod} for context.\end{sloppypar} All run in the same Julia process under identical
...
faster than the original LAMG on a large real-world test set. No implementation of the DRA-QC
variant is publicly distributed and we were unable to obtain the authors' code; we therefore
reimplemented it faithfully---validated to reproduce its published operator complexities and
$15$--$30$-iteration range---and emphasize that \emph{all DRA-QC figures below are from this
reimplementation}, not the original. We ran all three robust
near-linear solvers under identical build-once conditions over the full SuiteSparse corpus of
\S\ref{sec:scaling}.

All three are robust---each converges on $>99.7\%$ of the corpus (Table~\ref{tab:draqc})---but
LAMG+ is the only one that converges on \emph{every} graph, and with the tightest tail (at most
$27$ cycles, versus $77$ for approxChol and $277$ for DRA-QC); the stragglers are
anisotropy/Darcy/thermal matrices for DRA-QC and a single chemical-process matrix for approxChol.
Measured in work per edge (the identical operation count for LAMG+ and our DRA-QC reimplementation),
the two are \emph{complementary}: DRA-QC's leaner cycle does \emph{less} total work than LAMG+ on the
typical graph (median ratio $0.68$), but over $3\times$ on the $11\%$ of graphs with grid-aligned
anisotropy or high contrast (up to $55\times$), where aggregation-only coarsening lacks LAMG+'s
caliber-2 fix, while LAMG+'s worst-case work stays bounded. DRA-QC is thus a strong competitor with
a complementary cost profile---robust on real-world graphs with a proven bound---but without LAMG+'s
uniform convergence and bounded tail.
...
reimplementation, as no public code exists).}\label{tab:draqc}
...
LAMG+, in as few or fewer cycles and to a strict $10^{-8}$ residual. The reported failure is therefore an artifact of the original evaluation, not the
solver. (We could not reproduce the original evaluation's failure mechanism.)
