...
\title{A Reproducible Same-Machine Comparison of Graph-Laplacian Solvers,
  with Two Robustness Refinements to Lean Algebraic Multigrid\thanks{%
...
aggregate-quality control \cite{napov-notay} (the closest competitor, benchmarked in \S\ref{sec:draqc}), matching-based multilevel preconditioners
...
faster than the original LAMG on a large real-world test set. No public implementation exists and
we were unable to obtain the authors' code; we therefore reimplemented DRA-QC faithfully from the
...
reimplementation}, not the original code, and should be read accordingly. All comparisons in this
